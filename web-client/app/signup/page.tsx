...
import { Video, Loader2, Github, Facebook, Check, X, ShieldCheck } from 'lucide-react';
...
export default function Signup() {
  ...
  // 1. Instant Strength Checker
  useEffect(() => {
    const p = formData.password;
    let score = 0;
    if (!p) { setStrength(0); return; }

    if (p.length > 7) score += 1; // Length
    if (/[A-Z]/.test(p)) score += 1; // Uppercase
    if (/[0-9]/.test(p)) score += 1; // Number
    if (/[^A-Za-z0-9]/.test(p)) score += 1; // Special Char
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // We combine names before sending to backend to match your DB schema
      const payload = {
    ...
    <div className="min-h-screen bg-[#0B0F19] flex flex-col justify-center items-center p-6 relative overflow-hidden">
      ...
      <div className="w-full max-w-lg bg-[#131926] border border-white/10 rounded-3xl p-8 shadow-2xl backdrop-blur-xl relative z-10">
        <div className="text-center mb-6">
          <h2 className="text-2xl font-bold text-white mb-2">Secure Registration</h2>
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div className="space-y-1.5">
            ...
            <div className="relative">
              <input 
                required
                type="password" 
                className={`w-full bg-[#0B0F19] border rounded-xl px-4 py-3.5 text-white placeholder:text-slate-600 focus:ring-2 outline-none transition-all ${
              ...
              />
              {passwordsMatch && formData.confirmPassword && (
                <div className="absolute right-4 top-3.5 text-green-500 animate-in fade-in zoom-in">
          ...
          <button 
            type="submit" 
            disabled={loading || strength < 3 || !passwordsMatch}
            className="w-full bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-500 hover:to-indigo-500 text-white font-bold py-3.5 rounded-xl transition-all shadow-lg shadow-blue-900/20 flex items-center justify-center disabled:opacity-50 disabled:grayscale disabled:cursor-not-allowed mt-4 group"
          >
